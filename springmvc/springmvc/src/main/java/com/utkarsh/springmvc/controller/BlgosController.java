...
public class BlgosController {
	...
	protected String sort(@RequestParam("index")int index, HttpSession httpSession,ModelMap modelMap) {
		
		List<BlogsDTO> sortblogs = blogServices.sortblogs(index);
		User user = (User) httpSession.getAttribute("user");
		if (user != null) {
		if((user.getRole()).equals(Role.ADMIN)) {
		modelMap.addAttribute("user", user);
		modelMap.addAttribute("allblogs", sortblogs);
		return "manageblogs";		}
		else if((user.getRole()).equals(Role.USER)) {
			List<BlogsDTO> sortblogs1 = blogServices.sortblogs(index,httpSession);
			modelMap.addAttribute("blogs", sortblogs1);
			return "myblogs";
		}}
		else {
			modelMap.addAttribute("allblogs", sortblogs);
	...
	@RequestMapping(value = "/searchqurey",method = RequestMethod.GET)
	protected String getSeacrhedBlocks(@RequestParam("qvalue") String query,HttpSession httpSession,ModelMap modelMap) {
		
		List<BlogsDTO> searchblog = blogServices.searchblog(query);
		User user =(User) httpSession.getAttribute("user");
		if((user.getRole()).equals(Role.ADMIN)) {
		if(searchblog!=null)
		{
			
			modelMap.addAttribute("allblogs", searchblog);
			modelMap.addAttribute("user", user);
			...
			List<BlogsDTO> blogsofusers =  new ArrayList<>();
			
			for (BlogsDTO blogs : searchblog) {
				if(blogs.getUserid() == user.getId())
				{
					blogsofusers.add(blogs);
				}
			}
			modelMap.addAttribute("blogs", blogsofusers);
			modelMap.addAttribute("user", user);
			return "myblogs";
		}
